...
def graph_family(family_name, cplex_times, nr_times, bsc_times, nr_iters_, bs_iters_, cplex_infeas_):
    ...
    plt.xlabel('Number of nodes (logscale)', fontsize=12)
    plt.yscale('log')
    plt.xticks([])
    ...
    for atype in types:
        ...
        for exponent in exponents:
            ...
            for tail in tails:
                ...
                def am_i_close(a,b, rel_tol):
                    if abs(a - b) / min(a, b) <= rel_tol:
                        return True
                    else:
                        return  False
                ...
                for nr_ind in range(len(nr_iter_objs)-1, 0, -1):
                    # if not am_i_close(nr_iter_objs[nr_ind], nr_iter_objs[nr_ind-1], rel_tol=1e-8):
                    if not am_i_close(nr_iter_objs[nr_ind], nr_iter_objs[nr_ind-1], rel_tol=1e-8):
                ...
                for bs_ind in range(len(bs_iter_objs)-1, 0, -1):
                    if not am_i_close(bs_iter_objs[bs_ind], bs_iter_objs[bs_ind-1], rel_tol=1e-8):
                        bs_iter_objs = bs_iter_objs[:bs_ind+1]
                        bs_iter_elapsed = bs_iter_elapsed[:bs_ind+1]
                        break

                # for cp_ind in range(len(cplex_iter_objs)-1, 0, -1):
                #     if not am_i_close(cplex_iter_objs[cp_ind], cplex_iter_objs[cp_ind-1], rel_tol=1e-10):
                ...
                def get_indices(rel_tol, algo, iter_objs, min_obj=None):
                    ...
                    if trial:
                        ...
                        if algo == 'cplex' and (iter_objs[0] < iter_objs[1]):
                            for p in range(len(iter_objs)-1, 0, -1):
                                if iter_objs[p] < min_obj:
                                    ind = p+1
                                    break 

                                elif iter_objs[p] == min_obj:
                                    ind = p
                                    break 
                        else:
                            for p in range(len(iter_objs)-1, 0, -1):
                        ...
                        if algo=='nr':
                            for p in range(len(iter_objs)):
                                if am_i_close(min_obj, iter_objs[p], rel_tol=rel_tol):
                                    ind = p
                                    break
                            ...
                            if iter_objs[0] > iter_objs[1]:
                                ...
                                for p in range(len(iter_objs)):
                                    if am_i_close(min_obj, cplex_iter_objs[p], rel_tol=rel_tol):
                                        ind = p
                                        break
                            else:
                                for p in range(len(iter_objs)-1, 0, -1):
                                    if not am_i_close(iter_objs[p], iter_objs[p-1], rel_tol=rel_tol):
                                        ind = p
                                        break
                ...
                def find_indices(iter_objs, algo='nr', min_obj=None):
                    ind = get_indices(rel_tol, algo, iter_objs, min_obj)

                    # if ind is None:
                    #     pdb.set_trace()
                    #     ind = get_indices(rel_tol*5, algo, min_obj)

                    return ind


                if data_dic['solver_infeas'] < 1e-6:
                    # min_obj = min(min(nr_iter_objs), min(bs_iter_objs), cplex_obj)

                    common = max(min(nr_iter_objs), min(bs_iter_objs), cplex_obj)
                ...
                #     if not am_i_close(cplex_iter_objs[idx], nr_iter_objs[nr_i], rel_tol=1e-9):
                #         cut_idx = idx+1
                #         break

                # if cut_idx == None:
                #     for idx in range(len(cplex_iter_objs)-1, 0, -1):
                #         if not am_i_close(cplex_iter_objs[idx], nr_iter_objs[nr_i], rel_tol=1e-9*10):
                #             cut_idx = idx+1
                #             break
        ...
        for lam in lams:
            ...
            for exponent in exponents:
                for tail in tails:
                    ...
                    for nr_ind in range(len(nr_iter_objs)-1, 0, -1):
                        if not am_i_close(nr_iter_objs[nr_ind], nr_iter_objs[nr_ind-1], rel_tol=1e-9):
                            nr_iter_objs = nr_iter_objs[:nr_ind+1]
                            nr_iter_elapsed = nr_iter_elapsed[:nr_ind+1]
                            break
                    ...
                    for bs_ind in range(len(bs_iter_objs)-1, 0, -1):
                        if not am_i_close(bs_iter_objs[bs_ind], bs_iter_objs[bs_ind-1], rel_tol=1e-9):
                            bs_iter_objs = bs_iter_objs[:bs_ind+1]
                            bs_iter_elapsed = bs_iter_elapsed[:bs_ind+1]
                            break
                    
                    for cp_ind in range(len(cplex_iter_objs)-1, 0, -1):
                        if not am_i_close(cplex_iter_objs[cp_ind], cplex_iter_objs[cp_ind-1], rel_tol=1e-9):
                            cplex_iter_objs = cplex_iter_objs[:cp_ind+1]
                            cplex_iter_times = cplex_iter_times[:cp_ind+1]
                            break
                    ...
                    def get_indices(rel_tol, algo='nr'):
                        ...
                        if algo=='nr':
                            for p in range(len(nr_iter_objs)):
                                if am_i_close(cplex_obj, nr_iter_objs[p], rel_tol=rel_tol):
                                    ind = p
                                    break

                        else:
                            for p in range(len(bs_iter_objs)):
                                if am_i_close(cplex_obj, bs_iter_objs[p], rel_tol=rel_tol):
                                    ind = p
                                    break
                    ...
                    def find_indices(iter_objs, algo='nr'):
                        ...
                        if not am_i_close(cplex_obj, min(iter_objs), rel_tol=rel_tol):
                            ind = get_indices(rel_tol*10, algo)

                            if not am_i_close(cplex_obj, min(iter_objs), rel_tol=rel_tol*10):
                                ind = get_indices(rel_tol*100, algo)

                                if not am_i_close(cplex_obj, min(iter_objs), rel_tol=rel_tol*100):
